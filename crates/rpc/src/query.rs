...
use tendermint::v0_38::abci::response;
...
fn parse_deliver_tx(res: AbciQuery) -> anyhow::Result<response::FinalizeBlock> {
    let bz: Vec<u8> =
        fvm_ipld_encoding::from_slice(&res.value).context("failed to decode IPLD as bytes")?;

    let finalize_block = tendermint_proto::abci::ResponseFinalizeBlock::decode(bz.as_ref())
        .context("failed to deserialize ResponseDeliverTx from proto bytes")?;

    let mut finalize_block = response::FinalizeBlock::try_from(finalize_block)
        .context("failed to create DeliverTx from proto response")?;

    // Mimic the Base64 encoding of the value that Tendermint does.
    // finalize_block.data = encode_data(&finalize_block.data);
