...
use tendermint_rpc::{Client, HttpClient, Scheme, Url, endpoint::abci_query::AbciQuery};
use tendermint_rpc::{WebSocketClient, WebSocketClientDriver, WebSocketClientUrl};
use metis_vm::message::query::{FvmQuery, FvmQueryHeight};
use metis_vm::message::chain::ChainMessage;
    ...
    C: Client + Sync + Send,
{
    ...
    where
        F: FnOnce(&abci::types::ExecTxResult) -> anyhow::Result<T> + Sync + Send,
    {
...
where
    C: Client + Sync + Send,
{
    async fn perform<F, T>(&self, msg: ChainMessage, _f: F) -> anyhow::Result<SyncResponse<T>>
    where
        F: FnOnce(&abci::types::ExecTxResult) -> anyhow::Result<T> + Sync + Send,
    {
        let data = MessageFactory::serialize(&msg)?;
