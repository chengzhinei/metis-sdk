...
use metis_crypto::SecretKey;
use crate::actor_interface::{eam, evm};
...
use crate::message::conv::from_fvm;
...
impl SignedMessage {
    ...
    pub fn domain_hash(
        ...
        if maybe_eth_address(&self.message.from).is_some() {
            ...
            // let hash = cid::multihash::Multihash::Keccak256.digest(&rlp);
            let hash = Multihash::<32>::from_bytes(&rlp).unwrap();
...
#[cfg(test)]
mod tests {
    use crate::actor_interface::eam::EthAddress;
